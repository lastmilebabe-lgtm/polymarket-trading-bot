    ...
    while True:
        try:
            ...
            if slug != current_slug:
                ...
                # Fallback to next window if current not ready
                ...
                if not market:
                    print("⚠️ No market data found. Waiting 30s...")
                    await asyncio.sleep(30)
                    continue

                clob_tokens = market.get("clobTokenIds", [])
                print(f"🔍 clobTokenIds received: {clob_tokens} (type: {type(clob_tokens)})")

                if not isinstance(clob_tokens, list) or len(clob_tokens) < 2:
                    print("⚠️ Not enough valid clobTokenIds yet. Waiting...")
                    await asyncio.sleep(30)
                    continue

                up_token = str(clob_tokens[0]).strip()
                down_token = str(clob_tokens[1]).strip()

                if not up_token or not down_token or len(up_token) < 10:
                    print("⚠️ Token IDs look invalid. Waiting...")
                    await asyncio.sleep(30)
                    continue

                current_slug = slug
                print(f"✅ Ready! {slug} | Up token: {up_token[:16]}... | Down token: {down_token[:16]}...")

            # === Fetch BUY prices from CLOB (with better error handling) ===
            ...
            try:
                ...
                print(f"⚠️ Up price fetch failed: {e}")
            ...
            try:
                ...
                print(f"⚠️ Down price fetch failed: {e}")
            ...
            if total <= cheap_threshold:
                print(f"🚀 CHEAP! Total {total:.4f} — Buy both sides now")
                # TODO: Insert your buy logic here, e.g.
...
def fetch_market_by_slug(slug: str):
    """Fetch market using Gamma (still the reliable way for slug-based 15m markets)"""
    urls_to_try = [
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if isinstance(data, dict):
                    print(f"   → Market found for {slug}")
                    return data
        except Exception as e:
            print(f"   → Request failed: {e}")

    print(f"❌ Failed to fetch market for slug: {slug}")
    return None
